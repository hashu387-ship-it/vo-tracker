...
export function PaymentStats({ payments }: PaymentStatsProps) {
    ...
    const chartData = [...payments]
        .sort((a, b) => (a.id - b.id)) // Assuming ID order roughly matches sequence
        .map(p => ({
            name: p.paymentNo.replace('IPA ', ''), // Shorten for X-axis
            fullName: p.paymentNo,
            Gross: p.grossAmount || 0,
            Net: p.netPayment || 0,
            date: p.invoiceDate ? new Date(p.invoiceDate).toLocaleDateString('en-US', { month: 'short', year: '2-digit' }) : ''
        }));
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500">
            ...
            <Card className="col-span-4 bg-white dark:bg-gray-950 border-gray-200 dark:border-gray-800 shadow-sm">
                <CardHeader>
                    <CardTitle>Monthly Payment Progress</CardTitle>
                    <CardDescription>
                        Tracking Gross Certified vs Net Payment per application
                    </CardDescription>
                </CardHeader>
                <CardContent>
